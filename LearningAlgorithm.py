...
class LearningAlgorithm:
    ...
    def update_q(self,
        ...
        return self.apply_algorithm(q_table, state, action, reward, next_state, next_action, agent, world)



    def apply_algorithm(self, q_table, state, action, reward, next_state, next_action, agent, world):
        
        if(self.algorithm == my_enums.Algorithm.QLEARNING):
            return self.q_formula(q_table, state, action, reward, next_state, agent, world)
        else:
            return self.sarsa_formula(q_table, state, action, reward, next_state, next_action)


    def q_formula(self, q_table, state, action, reward, next_state, agent, world):
    ...
    def sarsa_formula(self, q_table, state, action, reward, next_state, next_action):
